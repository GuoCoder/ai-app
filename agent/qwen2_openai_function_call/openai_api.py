...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if query is _TEXT_COMPLETION_CMD:
        ...
        #     **gen_kwargs,
        # )
        response, _ =  model_chat(model, tokenizer, query, system, history,**gen_kwargs)
        print('<chat>')
...
def model_chat(model, tokenizer, prompt, system=None, history=None,**model_inputs):
    if(len(history) == 0):
        if(system == None):
            system = "You are a helpful assistant."
...
def _get_args():
    ...
    parser.add_argument(
        ...
        '--checkpoint-path',
        type=str,
        default='/workspace/guo/models/qwen/Qwen2-7B-Instruct',
