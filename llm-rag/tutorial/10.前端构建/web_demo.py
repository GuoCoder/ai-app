...
def predict(input, chatbot, max_length, top_p, temperature, history, past_key_values):
    ...
    content = vector.query(input, EmbeddingModel=embedding, k=3)[0]
    print(f'知识库输出：{content}')
    chat = GLM4Chat()
    response = chat.chat(input, history, content)
    history.append({"role": response.role, "content": response.content})
    chatbot[-1] = (parse_text(input), parse_text(response.content))
...
with gr.Blocks() as demo:
    gr.HTML("""<h1 align="center">大模型RAG构建</h1>""")
